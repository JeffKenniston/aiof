...
// ChaosAgent implements the Agent interface for Chaos Engineering and resilience testing.
type ChaosAgent struct {
	logger *slog.Logger
	client *genai.Client
}

func NewChaosAgent(logger *slog.Logger, client *genai.Client) *ChaosAgent {
	return &ChaosAgent{logger: logger, client: client}
}

func (c *ChaosAgent) handleToolCall(ctx context.Context, host AgentHost, toolName string, args map[string]interface{}, projectDir string, sandboxEnv *sandbox.MCPContainer, proposedChanges *string) (string, bool) {
	var result string
	isComplete := false
	agentName := "chaos_agent"

	if toolName == "TaskComplete" {
		host.EmitLog(ctx, agentName, "info", "Chaos TaskComplete requested.")
		isComplete = true
...
func (c *ChaosAgent) Execute(ctx context.Context, host AgentHost, input string, stateMutations []byte) (string, error) {
	c.logger.Info("ChaosAgent executing", slog.String("input", input))
	agentName := "chaos_agent"
	projectDir := host.GetProjectDir()

	if c.client == nil { return "", fmt.Errorf("ChaosAgent requires genai.Client") }

	host.EmitLog(ctx, agentName, "info", fmt.Sprintf("Chaos Agent started task: %s", input))
	host.EmitState(ctx, "agent_graph_state", agentName, fmt.Sprintf(`{"id": "%s", "label": "Chaos Monkey", "model": "Pro", "color": "#ff5555", "status": "running"}`, agentName))

	config := &genai.GenerateContentConfig{Tools: toolRegistry}
	sysPrompt := fmt.Sprintf(`You are an expert Chaos Engineering agent named %s.
Your task: %s
Working directory: %s

You must focus exclusively on resilience testing, dropping packets, killing microservices, and injecting latency.
Ensure the system handles cascading failures correctly.
Always verify your steps. When done, call TaskComplete.`, agentName, input, projectDir)
	...
	for i := 0; i < 150; i++ {
		resp, err := chat.SendMessage(ctx, genai.Part{Text: "Proceed with your chaos engineering task. Remember to use tools. If done, call TaskComplete."})
		if err != nil {
			host.EmitState(ctx, "agent_graph_state", agentName, fmt.Sprintf(`{"id": "%s", "label": "Chaos Monkey", "status": "error"}`, agentName))
	...
	host.EmitState(ctx, "agent_graph_state", agentName, fmt.Sprintf(`{"id": "%s", "label": "Chaos Monkey", "model": "Pro", "color": "#50fa7b", "status": "success"}`, agentName))
