...
// SREAgent implements the Agent interface for eBPF, Telemetry, and observability debugging.
type SREAgent struct {
	logger *slog.Logger
	client *genai.Client
}

func NewSREAgent(logger *slog.Logger, client *genai.Client) *SREAgent {
	return &SREAgent{logger: logger, client: client}
}

func (s *SREAgent) handleToolCall(ctx context.Context, host AgentHost, toolName string, args map[string]interface{}, projectDir string, sandboxEnv *sandbox.MCPContainer, proposedChanges *string) (string, bool) {
	var result string
	isComplete := false
	agentName := "sre_agent"

	if toolName == "TaskComplete" {
		host.EmitLog(ctx, agentName, "info", "SRE TaskComplete requested.")
		isComplete = true
...
func (s *SREAgent) Execute(ctx context.Context, host AgentHost, input string, stateMutations []byte) (string, error) {
	s.logger.Info("SREAgent executing", slog.String("input", input))
	agentName := "sre_agent"
	projectDir := host.GetProjectDir()

	if s.client == nil { return "", fmt.Errorf("SREAgent requires genai.Client") }

	host.EmitLog(ctx, agentName, "info", fmt.Sprintf("SRE Agent started task: %s", input))
	host.EmitState(ctx, "agent_graph_state", agentName, fmt.Sprintf(`{"id": "%s", "label": "SRE (eBPF)", "model": "Pro", "color": "#f8f8f2", "status": "running"}`, agentName))

	config := &genai.GenerateContentConfig{Tools: toolRegistry}
	sysPrompt := fmt.Sprintf(`You are an expert Site Reliability Engineer (SRE) agent named %s.
Your task: %s
Working directory: %s

You must focus exclusively on OpenTelemetry, eBPF logs, and generating/analyzing flamegraphs for system stability.
Always verify your steps. When done, call TaskComplete.`, agentName, input, projectDir)
	...
	for i := 0; i < 150; i++ {
		resp, err := chat.SendMessage(ctx, genai.Part{Text: "Proceed with your observability task. Remember to use tools. If done, call TaskComplete."})
		if err != nil {
			host.EmitState(ctx, "agent_graph_state", agentName, fmt.Sprintf(`{"id": "%s", "label": "SRE (eBPF)", "status": "error"}`, agentName))
	...
	host.EmitState(ctx, "agent_graph_state", agentName, fmt.Sprintf(`{"id": "%s", "label": "SRE (eBPF)", "model": "Pro", "color": "#50fa7b", "status": "success"}`, agentName))
