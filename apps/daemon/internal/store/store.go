...
// GetLatestDocument fetches the latest document of a given type.
func (s *Store) GetLatestDocument(ctx context.Context, docType string) (*Document, error) {
	if s.ephemeral {
		return nil, nil
	}
	query := `
		SELECT id, document_type, payload, updated_at, _deleted, valid_time_start, valid_time_end, system_time, provenance
		FROM documents
		WHERE document_type = $1 AND _deleted = false
		ORDER BY updated_at DESC
		LIMIT 1
	`
	var doc Document
	err := s.pool.QueryRow(ctx, query, docType).Scan(
		&doc.ID, &doc.DocumentType, &doc.Payload, &doc.UpdatedAt, 
		&doc.IsDeleted, &doc.ValidTimeStart, &doc.ValidTimeEnd, 
		&doc.SystemTime, &doc.Provenance,
	)
	if err != nil {
