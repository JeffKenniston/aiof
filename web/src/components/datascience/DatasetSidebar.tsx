...
export default function DatasetSidebar({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    fetch('http://localhost:8080/api/files')
      ...
      .then(data => {
        if(data.files) {
          setSources(data.files.slice(0, 10)); // Just show some real files as data sources
        }
      })
      .catch(console.error);
  }, []);
        ...
        <div className="space-y-2">
          {sources.length > 0 ? (
            sources.map((source, i) => (
              <div key={i} className="p-2 border border-white/10 rounded-lg text-sm text-text-primary flex justify-between items-center">
                <span className="truncate">{source.name}</span>
                <span className="text-[10px] text-text-muted">{(source.size/1024).toFixed(1)}kb</span>
              </div>
            ))
          ) : (
